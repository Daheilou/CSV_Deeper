...
def main():
    ...
    parser.add_argument("--resize_target", type=int, default=224)
    ...
    optimizer = AdamW([{'params': [p for p in model.backbone.parameters() if p.requires_grad], 'lr': args.base_lr},
                        {'params': [param for name, param in model.named_parameters() if 'backbone' not in name], 'lr': args.base_lr * 40}], 
                        lr=args.base_lr, betas=(0.9, 0.999), weight_decay=0.01)
    
    
    for param_group in optimizer.param_groups:
        param_group['initial_multiplier'] = param_group['lr'] / args.base_lr
...
def train_one_epoch(
    ...
    for i, (
        ...
        poly_ratio = (1 - iters / total_iters) ** 0.9
        optimizer.param_groups[0]["lr"] = poly_ratio * args.base_lr
        
        for param_group in optimizer.param_groups:
            # 获取初始倍数 (如果 main 中没加这行，默认回退到 1.0，防止崩溃)
            multiplier = param_group.get('initial_multiplier', 1.0)
            
            # 计算并更新学习率
            param_group["lr"] = args.base_lr * multiplier * poly_ratio
