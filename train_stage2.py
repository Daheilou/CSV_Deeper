...
def export_onnx(model, dummy_inputL, dummy_inputT, onnx_path, opset=13):
    model.eval()
    with torch.no_grad():
        torch.onnx.export(
            model,
            (dummy_inputL, dummy_inputT),  # 注意：TwoView 接收两个输入
            onnx_path,
            export_params=True,
            opset_version=opset,
            do_constant_folding=True,
            input_names=['long_view', 'trans_view'],
            output_names=['seg_long_logits', 'seg_trans_logits', 'cls_logits'],
            dynamic_axes={
                'long_view': {0: 'batch', 2: 'height', 3: 'width'},
                'trans_view': {0: 'batch', 2: 'height', 3: 'width'},
                'seg_long_logits': {0: 'batch', 2: 'height', 3: 'width'},
                'seg_trans_logits': {0: 'batch', 2: 'height', 3: 'width'},
                'cls_logits': {0: 'batch'}
            }
        )
...
def main():
    ...
    parser.add_argument("--resize_target", type=int, default=336)
    ...
    parser.add_argument("--model", type=str, default="Echocare", choices=["Echocare", "UNet", "dpt"],
    ...
    state_dict = torch.load(f'./dinov2_vits14_pretrain.pth')
    model.backbone.load_state_dict(state_dict)
    
    logger.info("Total params: {:.1f}M".format(count_params(model)))
    model = model.to(device)
    
    # model = load_checkpoint(model, './checkpoints/best.pth', device)
    model = load_checkpoint(model, './checkpoints/best_seg.pth', device) 
    ...
    optimizer = AdamW([{'params': [p for p in model.backbone.parameters() if p.requires_grad], 'lr': args.base_lr},
                        {'params': [param for name, param in model.named_parameters() if 'backbone' not in name], 'lr': args.base_lr * 50}], 
    ...
    output_dict = validate(args, model_ema, valid_loader, device, logger, writer=writer, epoch=0)
    ...
    for epoch in range(start_epoch, args.train_epochs):
        ...
        output_dict = validate(args, model_ema, valid_loader, device, logger, writer=writer, epoch=epoch)
        ...
        if is_best:
            ...
            logger.info(f"New best! total_score={total_score:.3f} saved to best.pth")
#             onnx_path = os.path.join(args.save_path, "best.onnx")
#             resize = 256  # e.g., 256
#             dummy_L = torch.randn(1, 1, resize, resize).to(device)  # [B, C, H, W]
#             dummy_T = torch.randn(1, 1, resize, resize).to(device)

#             export_onnx(model, dummy_L, dummy_T, onnx_path, opset=13)
...
def validate(args, model, valid_loader, device, logger, writer=None, epoch=None):
    ...
    f1 = f1_score(cls_gt, cls_pred, average='macro')
...
def get_model(args):
    ...
    if args.model == "Echocare":
        ...
        model = DualViewDPT(
            encoder_size='small',
            in_chns=1,
            seg_nclass=args.seg_num_classes,
            cls_nclass=args.cls_num_classes,
        )  
